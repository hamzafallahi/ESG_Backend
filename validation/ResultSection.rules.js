...
const ALLOWED_SORT_FIELDS = ["id", "result_id", "section_id", "score", "level", "created_at", "updated_at"];

const resultSectionDataSchema = {
  type: Joi.string().valid("ResultSection").required(),
...
const resultSectionUpdateDataSchema = {
  type: Joi.string().valid("ResultSection").required(),
  attributes: Joi.object()
    .keys({
...
module.exports = {
  create: {
    ...
    body: Joi.object()
      .keys({
        data: Joi.object().keys(resultSectionDataSchema).required(),
      })
  ...
  update: {
    ...
    body: Joi.object()
      .keys({
        data: Joi.object().keys(resultSectionUpdateDataSchema).required(),
      })
      .options({ abortEarly: false }),
  },
  getAll: {
