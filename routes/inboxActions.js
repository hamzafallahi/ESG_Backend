...
router.post('/retake-request', authenticate, requireUser, validate(retakeRequest), inboxActionsController.requestRetake);

// User submits a bug report
// POST /inbox-actions/bug-report
router.post('/bug-report', authenticate, requireUser, validate(bugReport), inboxActionsController.submitBugReport);

// User submits a support request
// POST /inbox-actions/support-request
router.post('/support-request', authenticate, requireUser, validate(supportRequest), inboxActionsController.submitSupportRequest);
